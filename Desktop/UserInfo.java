...
public class UserInfo {
	...
	void updateUserInfo() throws IOException
	{
		...
		if(!idCheck){
			...
			System.out.println("학생정보 조회로 한번 더 확인해주세요:)");
			mainScreen();
		}
		FileWriter fw = null;
		BufferedWriter bw = null;
		...
		System.out.println("=>전화번호 외의 다른 정보도 수정가능합니다.");
		Id = br.readLine();
		name = br.readLine();
		department = br.readLine();
		Phone_number = br.readLine();
	...
	void deleteUserInfo() throws IOException{
		...
		if(!idCheck){
			...
			System.out.println("학생정보에서 확인해주세요^^");
			mainScreen();
			//deleteUserInfo();
		}
		FileWriter fw = null;
	...
	void searchUserInfo() throws IOException {
		...
		for(int i=0; i<v.size(); i++){
			...
			if(Id.equals(idtest))
			{
				System.out.println("검색하신 학번의 학생정보는 아래와 같습니다");
				System.out.println("학     번: "+ test.get("Id"));
				System.out.println("이     름: "+ test.get("name"));
				System.out.println("전     공: "+ test.get("department"));
				System.out.println("전화번호: "+ test.get("Phone_number"));
				idCheck=true;
		...
		if(!idCheck){
			...
			System.out.println("본 메뉴화면으로 돌아갑니다");
		}
		v.clear();
	}
